...
# Configuration Options for `wget`
WGET = {
...
def download_httpd():
    ...
    except subprocess.CalledProccessError as CPEErr:
        print('ERROR::', CPEErr)
...
try:
    ...
    subprocess.check_call('sudo make', shell=True)
    # subprocess.check_call('sudo make install', shell=True)
    p = subprocess.Popen(['sudo', 'make', 'install'], stdout=subprocess.PIPE, stdin=subprocess.PIPE, stderr=subprocess.STDOUT)    
    grep_stdout = p.communicate(input=b'y')[0]
except subprocess.CalledProcessError as CPEErr:
    print('ERROR::', CPEErr)
